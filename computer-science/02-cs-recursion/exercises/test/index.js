...
describe("lengthOfString", () => {
  it("should find the length of a given string", () => {
    let testStr = "Chris";
    expect(lengthOfString(testStr)).to.equal(5);
  });
  it("should work for a string with length of 0", () => {
    let noLength = "";
    expect(lengthOfString(noLength)).to.equal(0);
  });
});

describe("sumOfArray", () => {
  it("should find the sum of the numbers in an array", () => {
    let testArray = [3, 4, 2, 1, 2];
    expect(sumOfArray(testArray)).to.equal(12);
  });
  it("should work for negative numbers", () => {
    let testNegatives = [-1, -4, -2];
    expect(sumOfArray(testNegatives)).to.equal(-7);
  });
});
