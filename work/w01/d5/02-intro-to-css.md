...
News Flash: EVERYTHING you see on a web page is a BOX (rectangle)!

When the browser positions and "paints" elements on the page, it does so according to the following four parts that each element has:

<img src="https://i.imgur.com/wt3yR8Y.png">

**Content Area**:

- Contains the actual content of the element including its text and other nested elements.
- By default, the `box-sizing` property is set to a value of `content-box` and results in the `height` and `width` properties sizing the Content Area.  However, it is often preferable to change the value to `border-box` so that the element's `width` and `height` include everything except the Margin Area.

**Padding Area**:

- The area that separates the content from the element's border.
...
**1. When there's a conflict between internal and/or external stylesheets, who wins?**
