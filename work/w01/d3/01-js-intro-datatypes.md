...
Variables are a way to name a "slot" in computer memory used to hold data...
...
The difference between `var` and `let`/`const` is what we call _scope_. Scope revolves around the accessibility of variables & functions at a given point in the code.

We're going to cover scope tomorrow, but feel free to refer to the Further Study section before then.
...
Note that the above graphic does not show the bigint primitive type.
