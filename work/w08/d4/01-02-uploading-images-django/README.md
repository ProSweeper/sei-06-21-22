...
To access the values in a Python app, we use the `os.environ` dict - similar to how we used Node's `process.env`:
...
We're going to need to update the **main_app/templates/cats/detail.html** template to:
...
Django templates provide [filters](https://docs.djangoproject.com/en/4.0/ref/templates/builtins/#built-in-filter-reference) for use in both `{{ }}` (variable) and `{% %}` (tags).
...
Filters can also be used to transform/format data, for example, here's how the [`capfirst`](https://docs.djangoproject.com/en/4.0/ref/templates/builtins/#capfirst) filter can be used to capitalize the first letter of an outputted value:

```html
{{ some_value|capfirst }}
```

or how the [`date`](https://docs.djangoproject.com/en/4.0/ref/templates/builtins/#date) filter can be used to format dates:
...
<!-- new code below -->
<form action="{% url 'add_photo' cat.id %}" enctype="multipart/form-data" method="POST" alt="Cat Photo" class="card-panel">
    {% csrf_token %}
...
[Django Built-in Template Tags and Filters](https://docs.djangoproject.com/en/4.0/ref/templates/builtins/#built-in-template-tags-and-filters)
