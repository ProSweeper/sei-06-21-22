...
GitHub has started to transition the name of the default branch from `master` to `main`.
...
- Run `git checkout -b gh-pages`
    > The `-b` option creates the branch if it does not exist. You can also accomplish the same thing with the following two commands: `git branch gh-pages` followed by `git checkout gh-pages`.
...
Because our browsers cache files, it may be necessary to load updated files such as CSS stylesheets, etc.

Use `shift + command + R` to do a hard-refresh
