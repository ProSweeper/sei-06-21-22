...
3. Let's open the project in VS Code: `code .`
	
4. Rename `app.js` to `server.js` and update the following line in **bin/www** as follows:
...
5. Just in case you need to sync during the code-along, let's make this project a repo and link to the code-along's remote:
...
### Sync Your Code As Needed

**IF** you need to sync during the code-along, here are the familiar commands:
